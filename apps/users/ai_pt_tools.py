            ...
            "properties": {
                ...
                "payload": {
                    ...
                    "description": (
                        ...
                        "\n"
                        "SWAP-MULTI-OPTION (preferred for swap_exercise):\n"
                        "If you have 2-3 sensible alternatives the user "
                        "might prefer (different equipment, different "
                        "mechanic, different difficulty), use the OPTIONS "
                        "array shape instead of a single new_exercise_name. "
                        "DO NOT fire 3 separate propose_workout_mutation "
                        "calls — that produces 3 cards the user has to "
                        "accept-or-deny individually, which is bad UX. "
                        "Multi-option shape:\n"
                        "  swap_exercise={\n"
                        "    current_exercise_name: '...',\n"
                        "    options: [\n"
                        "      {name: 'Cable Row',     sets: 3, reps: 10, rest_seconds: 90},\n"
                        "      {name: 'Bent-over Row', sets: 3, reps: 8,  rest_seconds: 120},\n"
                        "      {name: 'T-Bar Row',     sets: 3, reps: 8,  rest_seconds: 120}\n"
                        "    ]\n"
                        "  }\n"
                        "When you use options, write the rationale at the "
                        "PROPOSAL level (the user picks WHICH one fits "
                        "best from the list — your job is to explain the "
                        "category, not each option). Each option may "
                        "carry an optional 'hint' string (≤80 chars) "
                        "explaining when this specific option is the right "
                        "pick (e.g. 'best for limited grip', 'no machine "
                        "needed'). For a single-option swap with no "
                        "alternatives, the existing "
